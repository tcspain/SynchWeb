...
define(["underscore"],
	...
	var weightedComposition = function(phaseModel, abundance) {
		...
		var fimi = abundance/phaseModel.get("MOLECULARMASS");
		// Get the number of each element present
		var elementNumbers = mapFormula(flattenBrackets(phaseModel.get("SEQUENCE")));
	 ...
	 * to number of atoms
	 */
	var mapFormula = function(formula) {
		...
		var compositionHash = _.reduce(elements, 
			...
			return memo;
		}, {}, {"allTheElements": allTheElements});
		
		return compositionHash;
	};

	// FIXME
	var flattenBrackets = function(bracketedFormula) {
	...
	var stringifyElementHash = function(elementHash) {
		...
		formula = _.reduce(allElements, function(memo, element, index, list) {
			...
			"addElementToString": addElementToString,
			"elementHash": elementHash,
		});
		return formula;
	};
	
	var addElementToString = function(formula, element, elementHash) {
		formula += element;
	...
	var allTheElements = [
	 ...
	 return {
		 ...
		 * composition string.
		 */
		compositionComposite: function(phaseCollection, abundanceMap) {
			var totalFimi = 0;
//			var elements = {};
			...
			var elementsAndFimi = nonNullPhaseCollection.reduce(function(localElementMemo, phaseModel, key, list) {
				 ...
				 * weighting
				 */
				var result = weightedComposition(phaseModel, abundanceMap[phaseModel.get("PROTEINID")]);
			...
			var formula =  subscriptifyNumbers(stringifyElementHash(elementsAndFimi.elements));
			return formula;
		},
